...
$queryR = $db->query("SELECT * FROM `good` JOIN `category` ON good.cat_id=category.id WHERE good.status=1 AND category.status=1");
    ...
    <div class="cont catalog-wrapper">
        ...
        <div class="cart-page_header">
                    <ul>
                        ...
                        <li><span class="gray">Спортивное питание</span></li>
                    </ul>
                </div>
            <div class="catalog_cont" id="catcont">
                <?
                    $query = $db->query("SELECT g.id,g.name,g.cat_id,g.subcat_id,g.price,g.quantity,g.link_img,s.name AS namesubcat FROM `good`AS g JOIN `subcategory`AS s ON g.subcat_id=s.id JOIN `category` AS c ON c.id=g.cat_id WHERE g.status=1 AND c.status=1 LIMIT $limit OFFSET $offset ");
            ...
            <div class="pagin">
           <? include_once "$path/catalog/pagination.php"; ?></div>
        </div>
